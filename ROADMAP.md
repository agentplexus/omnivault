...
|------|--------|----------|------|
| HTTP server over Unix socket | ✅ | High Priority | Daemon |
| Secret list UI | 📋 | High Priority | UI |
| URI-based resolver | ✅ | High Priority | Core Library |
| External provider module architecture | ✅ | High Priority | Providers |
| Lock/unlock session management | ✅ | High Priority | Daemon |
| Swift state management | 📋 | High Priority | UI |
| AES-256-GCM encryption with Argon2id key derivation | ✅ | High Priority | Security |
| Encrypted file store implementing `vault.Vault` | ✅ | High Priority | Security |
| Master password protection | ✅ | High Priority | Security |
| Swift Unix socket client | 📋 | High Priority | UI |
| Platform-specific paths (~/.omnivault/) | ✅ | High Priority | Daemon |
| CLI entrypoint | ✅ | High Priority | CLI |
| Built-in providers (env, file, memory) | ✅ | High Priority | Providers |
| Core `vault.Vault` interface | ✅ | High Priority | Core Library |
| IPC protocol definitions | ✅ | High Priority | Daemon |
| Secret commands (get, set, list, delete) | ✅ | High Priority | CLI |
| Vault commands (init, lock, unlock, status) | ✅ | High Priority | CLI |
| Daemon control commands | ✅ | High Priority | CLI |
| Unix socket client | ✅ | High Priority | Client |
| Swift models for secret info and vault status | 📋 | High Priority | UI |
| Automatic daemon startup on first use | 📋 | Medium Priority | Client |
| Configurable auto-lock timeout | ✅ | Medium Priority | Security |
| omnivault-keyring provider module | ✅ | Medium Priority | Providers |
| omnivault-aws provider module | ✅ | Medium Priority | Providers |
| Lock/unlock UI | 📋 | Medium Priority | UI |
| Add/edit secret UI | 📋 | Medium Priority | UI |
| Connection pooling | 📋 | Low Priority | Client |
